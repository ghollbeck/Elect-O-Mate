...
def CreateCSV(file_path):
        ...
        # Put together CSV for all parties and Questions
        party_answers_array = np.zeros((num_unique_questions, party_names_length))
        for i, party in enumerate(party_names):
            party_answers = [item['Party_Answer'] for item in data_Party['party_answers'] if item['Party_Name'] == party]
            party_answers_array[:, i] = party_answers

        output_file_path = f"./Backend/Evals/Party_Answers_CSV/{file_path.split('/')[-1][-7:-5].upper()}_CSV.csv"
        np.savetxt(output_file_path, party_answers_array, delimiter=",", fmt='%d', header='', comments='')

        return party_answers_array, party_names_length, num_unique_questions
