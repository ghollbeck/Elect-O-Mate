import React from 'react';
import { useTranslation } from 'react-i18next';

const Top = ({ onButtonClick }) => {
  const { t } = useTranslation();
  return (
    <div className='md:mb-[100px]'>
      <header
        className='text-black  my-10 text-center'
        style={{ color: '#C5D5D3' }}
      >
        <h1 className='text-5xl md:text-8xl font-extrabold'>Elect-O-Mate</h1>
        <p className='mt-4'>{t('subheading')}</p>
        <button
          className='m-8 py-4 px-9 text-white rounded-full font-semibold  bg-gray-700/30 transition duration-300 ease-in-out transform hover:scale-110 text-xl shadow-lg'
          onClick={onButtonClick}
        >
          {t('start_button')}
        </button>{' '}
        {/*         <button
          className='m-8 py-4 px-9 text-black rounded-full font-semibold transition duration-300 ease-in-out transform hover:scale-110 text-xl backdrop-filter backdrop-blur-lg bg-gray-600 bg-opacity-50'
          onClick={onButtonClick}
        >
          {t('start_button')}
        </button> */}
      </header>
    </div>
  );
};

export default Top;
